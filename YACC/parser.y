...
/* -------------------------------------------------- */
%token TYPE END_TYPE VAR_INPUT RETAIN NON_RETAIN END_VAR BOOL R_EDGE F_EDGE VAR_OUTPUT VAR_IN_OUT VAR CONSTANT VAR_EXTERNAL VAR_GLOBAL AT VAR_ACCESS PROGRAM WITH TASK SINGLE INTERVAL PRIORITY READ_WRITE READ_ONLY VAR_CONFIG FUNCTION FUNCTION_BLOCK END_FUNCTION END_FUNCTION_BLOCK
%token L_CURLY R_CURLY
%token G_THAN G_EQ L_THAN L_EQ NOT_EQ EQ_OP
%token STAR SLASH PERCENT STAR_STAR
%token AMPERSAND XOR AMPERSAND_N OR_N XOR_N
%token LD LDN ST STN S R S1 R1 CLK CU CD PV IN PT
%token CAL CALC CALCN RET RETC RETCN JMP JMPC JMPCN
%token ELSIF ELSE END_IF CASE OF END_CASE FOR TO BY DO END_FOR WHILE END_WHILE REPEAT UNTIL END_REPEAT EXIT
...
%token BINARY_INTEGER OCTAL_INTEGER HEX_INTEGER BIT_STRING_LITERAL CHARACTER_STRING TIME_LITERAL DURATION_LITERAL DATE_LITERAL TIME_OF_DAY_LITERAL DATE_AND_TIME_LITERAL
